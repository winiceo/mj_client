...
function M:initItems()
    ...
    accountLabel:setColor(cc.BLUE)
	accountLabel:setPosition(self.frameSize.width/2 - 100, self.frameSize.height/2-80)
	self.bgLayer:addChild(accountLabel)
	
	local accountEdit= cc.EditBox:create({width = 201, height = 26}, "edit_bg.png")
    accountEdit:setPosition(self.frameSize.width/2 + 40 , self.frameSize.height/2-80)
    self.bgLayer:addChild(accountEdit)

	local passwdLabel = cc.Label:createWithSystemFont("账 号:", "arial", 28)
    passwdLabel:setColor(cc.BLUE)
	passwdLabel:setPosition(self.frameSize.width/2 - 100, self.frameSize.height/2-140)
	self.bgLayer:addChild(passwdLabel)
	
	local passwdEdit= cc.EditBox:create({width = 201, height = 26}, "edit_bg.png")
    passwdEdit:setPosition(self.frameSize.width/2 + 40 , self.frameSize.height/2-140)
    self.bgLayer:addChild(passwdEdit)

	local loginBtn = ccui.Button:create("register_btn.png")
    loginBtn:setScale(0.6)
	loginBtn:setAnchorPoint(cc.p(0.5, 0.5))
    loginBtn:setPosition(self.frameSize.width/2 - 80 , self.frameSize.height/2 - 200)
	loginBtn:setTitleText("登    陆")
	loginBtn:setTitleFontSize(35)
	self.bgLayer:addChild(loginBtn)
	
	local registerBtn = ccui.Button:create("register_btn.png")
    registerBtn:setScale(0.6)
	registerBtn:setAnchorPoint(cc.p(0.5, 0.5))
    registerBtn:setPosition(self.frameSize.width/2 + 80, self.frameSize.height/2 - 200)
